...
async def deleteServerPosts(serverID):
    try:
        pymongoManager.delete_all_by_query(
            'schedules', {'server_id': serverID})
    except RuntimeError as e:
        raise e
...
async def updateMessageObject(ctx, data):
    try:
        pymongoManager.update_collection(
            'messages', ctx.message.guild.id, data)
    except RuntimeError as e:
        raise e
...
async def validateDate(dateData):
    ...
    if not (len(date) == 3 and date[0].isdigit() and date[1].isdigit() and date[2].isdigit() and len(date[0]) == 2 and len(date[1]) == 2 and len(date[2]) == 4):
        raise ValueError("The date was not provided in dd/mm/yyyy format.")
    # hh:mm format check
    elif not (len(time) == 2 and time[0].isdigit() and time[1].isdigit() and len(time[0]) == 2 and len(time[1]) == 2):
...
def getSecondsFromNextMinute():
    try:
        now = datetime.now()
        nextMinute = datetime(now.year, now.month, now.day,
                              now.hour, now.minute, 0, 0) + timedelta(minutes=1)
...
async def handleAdd(ctx, rawArgs):
    ...
    dateObj = datetime.strptime(
        args[1] + 'T' + args[2] + ':00+00:00', dateFormat)
    ...
    try:
        ...
        await updateSchedule(postID, scheduleData)
    except RuntimeError as e:
        print(e)
...
async def handleRemove(ctx, msg):
    ...
    if not postID.isdigit():
        raise TypeError(
            f"Invalid post ID: {postID}. Post IDs may only contain numbers.")
    ...
    if not post:
        raise ValueError(
            f"There is no scheduled post with the corresponding post ID: {postID}")
    ...
    try:
        ...
        print(e)
        raise RuntimeError(
            f"Could not delete the post with ID: {postID}. The command will be ignored.")
...
async def handleSet(ctx, msg):
    ...
    msgObj['message'] = msg
    msgObj['attachments'] = {
        'message_id': ctx.message.id, 'channel_id': ctx.message.channel.id}
